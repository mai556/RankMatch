...
class DeepLabV3Plus(nn.Module):
    ...
    def forward(self, x, need_fp=False):
        ...
        c1, c4 = feats[0], feats[-1] # [4, 256, 129, 129], [4, 2048, 33, 33]
        ...
        out = F.interpolate(out, size=(h, w), mode="bilinear", align_corners=True) # ~4x upsample
    ...
    def _decode(self, c1, c4):
        ...
        feature = self.fuse(feature) # [4, 256, 129, 129]
...
class DeepLabV3Plus_modify(nn.Module):
    # return feature
    def __init__(self, cfg):
        super(DeepLabV3Plus_modify, self).__init__()
    ...
    def forward(self, x, need_fp=False):
        ...
        if need_fp:
            outs, features = self._decode(torch.cat((c1, nn.Dropout2d(0.5)(c1))),
                                torch.cat((c4, nn.Dropout2d(0.5)(c4))))
            outs = F.interpolate(outs, size=(h, w), mode="bilinear", align_corners=True)
            out, out_fp = outs.chunk(2)
